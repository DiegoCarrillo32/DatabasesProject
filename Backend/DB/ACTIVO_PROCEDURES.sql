...
CREATE OR ALTER PROCEDURE INSERT_ACTIVO(@ID_AREA SMALLINT,@NOMBRE_ACTIVO VARCHAR(20),@PLACA INT, @DESCRIPCION VARCHAR(150), @GARANTIA VARCHAR(20), @ID_UBICACION SMALLINT)
AS
    DECLARE @ERROR BIT;
    SET @ERROR = 'FALSE';
    BEGIN TRANSACTION
...
CREATE OR ALTER PROC DELETE_ACTIVO(@ID_ACTIVO SMALLINT)
AS  
    DECLARE @ERROR BIT;
    SET @ERROR = 'FALSE';
    BEGIN TRANSACTION
    ...
    ELSE
    BEGIN
        PRINT('SE ELIMINÓ ACTIVOS, CONTROL DE ACTVIOS Y TIPOS')
        COMMIT TRANSACTION
    END;
GO

CREATE OR ALTER PROC UPDATE_ACTIVO(@ID_ACTIVO SMALLINT,@ID_AREA SMALLINT,@NOMBRE_ACTIVO VARCHAR(20),@ID_UBICACION SMALLINT,@PLACA INT, @DESCRIPCION VARCHAR(150), @GARANTIA VARCHAR(20),@ESTADO_ACTIVO VARCHAR(30))
AS
    DECLARE
        @ERROR BIT;
    SET
